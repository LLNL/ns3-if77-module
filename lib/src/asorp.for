...
C     ROUTINE FOR MODEL APR 77        
   ...
   19 FORMAT(5X,'FREQUENCY IS TOO HIGH FOR ABSORPTION TABLE USING VALUES        
     ...
     X14,.10/           
      DATA ZW/12*0.0,.00000001,.0001,.00017,.00034,.0021,.009,.025,.045,
     W.10,.22,0.20,.16,.15,.11,.14,.10,.099,.098,.0963,.0967,.0981,.0987
     W,.099,.100,.101,.103,.109,.118,.120,.122,.127,.130,.132,.138,.154,
     W.161,.175,.20,.25,.34,.56/                
...
50    CONTINUE          
*pdb  WRITE(IOT,2010) (P(J),YA(J),YB(J),YC(J), J=1,35)            
...
C     -----------------------WRITE STATEMENTS-------------------- 
      IF(II.GT.0) GO TO 11            
*pdb  WRITE(IOT,51)     
*pdb  WRITE(IOT,52)DL7,DL8,TEC1,TEC2,TE4,AC3,D3,AC4,D4,AV4,GH7,ARK,AKS          
*pdb  CALL PAGE(2)      
   11 CONTINUE          
C     ----------------------------------------------------------- 
      AK4=AEK+DK4*AMKD                
...
C     -----------------------WRITE STATEMENTS-------------------- 
      IF(II.GT.0) RETURN              
*pdb  WRITE(IOT,60)     
*pdb  WRITE(IOT,6)AR3,AR4,DR3,DR4,AK3,AK4,D,DK4,GH1,GH2,W,AMD,AED,SWP,          
*pdb XAWD,AK5,DK5       
*pdb  CALL PAGE(2)      
C     ----------------------------------------------------------- 
      RETURN            
...
C     -----------------------WRITE STATEMENTS-------------------- 
      IF(II.GT.0) RETURN              
*pbd  WRITE(IOT,57)     
*pdb  WRITE(IOT,7)AK3,AK4,D,DK4,GH1,GH2,W,AMD,AED,SWP,AWD,AK5,DK5 
*pdb  CALL PAGE(2)      
C     ----------------------------------------------------------- 
      RETURN            
...
C     -----------------------WRITE STATEMENTS-------------------- 
      IF(II.GT.0) RETURN              
*pdb  WRITE(IOT,61)     
*pdb  WRITE(IOT,5)AR3,AR4,DR3,DR4,W,AMD,AED         
*pdb  CALL PAGE(2)      
C     ----------------------------------------------------------- 
      RETURN            
...
C     ----------------- JTAC -------------------------------------------        
   70 D=A*57.29577951                 
    ...
    1 GAV=GV            
      ...
      ENTRY ANTNA(X,ITA,HH,TT,GAV,GAH,GDR,NP)
      A=X               
      IFA=ITA           
      HPBW=HH           
      TLT=TT            
  ...
  550 GAIN=0.           
      RETURN            
 501  CONTINUE
*pdb  WRITE(IOT,551)PA1               
      GO TO 550         
  502 CONTINUE
*pdb  WRITE(IOT,552)PA2               
      GO TO 550         
  503 CONTINUE
*pdb  WRITE(IOT,553)PA3               
      GO TO 550         
  504 CONTINUE
*pdb  WRITE(IOT,554)PA4               
      GO TO 550         
  505 CONTINUE
*pdb  WRITE(IOT,555)PA5               
      GO TO 550         
  506 CONTINUE
*pdb  WRITE(IOT,556)PA6               
      GO TO 550         
  507 CONTINUE
*pdb  WRITE(IOT,557)PA7               
      GO TO 550         
        END             
...
*pdb  IF(AK.GT..1) WRITE(IOT,6)GHBAR,HB,AK,B        
*pdb  IF(AK.LT..01) WRITE(IOT,6)GHBAR,HB,AK,B       
*pdb  IF(HB.LT..01) WRITE(IOT,6)GHBAR,HB,AK,B       
*pdb  IF(HB.GT.100.) WRITE(IOT,6)GHBAR,HB,AK,B      
    5 RETURN            
